...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const loadTokens = async () => {
      try {
        ...
        const createdAtStr = await SecureStore.getItemAsync(TOKEN_CREATED_AT_KEY);
        const codeVerifier = await SecureStore.getItemAsync(CODE_VERIFIER_KEY);
        
        if (accessToken && refreshToken && expiresAtStr) {
          const expiresAt = parseInt(expiresAtStr, 10);
          const createdAt = createdAtStr ? parseInt(createdAtStr, 10) : Date.now();
          setTokensState({
            accessToken,
            refreshToken,
            expiresAt,
  ...
  const setTokens = async (newTokens: AuthTokens | null) => {
    try {
      if (newTokens) {
        ...
        await SecureStore.setItemAsync(EXPIRES_AT_KEY, newTokens.expiresAt.toString());
        await SecureStore.setItemAsync(TOKEN_CREATED_AT_KEY, newTokens.createdAt.toString());
        if (newTokens.codeVerifier) {
          await SecureStore.setItemAsync(CODE_VERIFIER_KEY, newTokens.codeVerifier);
        }
  ...
  const refreshAccessToken = async (): Promise<{ success: boolean; newTokens?: AuthTokens }> => {
    try {
      if (refreshPromise) {
      ...
      if (!process.env.EXPO_PUBLIC_CLIENT_ID) {
        console.error('Cannot refresh token: EXPO_PUBLIC_CLIENT_ID environment variable is not set');
        await forceLogout();
        return { success: false };
      }
      ...
      if (timeSinceLastRefresh < MIN_REFRESH_INTERVAL_MS) {
        console.log(`Skipping token refresh - last refresh was ${timeSinceLastRefresh}ms ago (minimum interval: ${MIN_REFRESH_INTERVAL_MS}ms)`);
      ...
      if (tokenAge < 10000 && !isTokenExpired) { 
        console.log(`Token was just created ${tokenAge}ms ago and isn't expired, skipping refresh`);
        return { success: true, newTokens: tokens };
      }
      ...
      refreshPromise = (async () => {
        try {
          ...
          const response = await fetch(`${process.env.EXPO_PUBLIC_API_BASE}/oauth/token`, {
            method: 'POST',
            headers: {
