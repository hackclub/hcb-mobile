    ...
    async (config) => {
      ...
      if (!fs.existsSync(privateSDKPath)) {
        ...
        try {
          ...
          console.log(
            "✅ Configured dependencies for EAS build (without private Maven repo)",
          );
        } catch (error) {
          console.error(
            "❌ Error configuring dependencies for EAS build:",
            error.message,
          );
        }
      ...
      try {
        ...
        await configureRootBuildGradle(
          rootBuildGradlePath,
          privateSDKPath,
          projectRoot,
        );
 ...
 */
async function configureRootBuildGradle(
  buildGradlePath,
  privateSDKPath,
  projectRootPath,
) {
  ...
  if (!privateSDKAvailable) {
    console.log(
      "⚠️  Private SDK not available - removing private Maven repository from root build.gradle",
    );

    // Remove any existing private Maven repository references
    const privateMavenRegex =
      /maven\s*\{\s*url\s*file\(['"][^'"]*private-sdk[^'"]*['"]\)\s*\}\s*\/\/\s*Google Play Services Tap and Pay private SDK[^\n]*\n?/g;
    if (buildGradleContent.match(privateMavenRegex)) {
      buildGradleContent = buildGradleContent.replace(privateMavenRegex, "");
      fs.writeFileSync(buildGradlePath, buildGradleContent);
      console.log(
        "✅ Removed private Maven repository from root build.gradle (private SDK not available)",
      );
    } else {
      console.log(
        "✅ No private Maven repository references found in root build.gradle",
      );
    }
    return;
  }
  ...
  if (
    buildGradleContent.includes(
      "// Google Play Services Tap and Pay private SDK",
    )
  ) {
  ...
  const repositoriesRegex =
    /(allprojects\s*\{[\s\S]*?repositories\s*\{[\s\S]*?)(google\(\)[\s\S]*?)(mavenCentral\(\))/;
...
async function verifyAppBuildGradle(buildGradlePath, privateSDKPath) {
  ...
  if (privateSDKAvailable) {
    requiredDependencies.push(
      {
        dependency:
          "implementation 'com.google.android.gms:play-services-tapandpay:17.1.2' // Google Play Services Tap and Pay for Push Provisioning",
        check: "play-services-tapandpay:17.1.2",
        name: "Google Play Services Tap and Pay",
      },
      {
        dependency:
          "implementation 'com.stripe:stripe-android-issuing-push-provisioning:1.1.0' // Stripe Android Issuing Push Provisioning",
  ...
  const hermesBlockRegex =
    /(if \(hermesEnabled\.toBoolean\(\)\) \{[\s\S]*?\} else \{[\s\S]*?\}\s*)(\})/;
  ...
  if (!privateSDKAvailable) {
    ...
    for (const dep of privateDependencies) {
      const regex = new RegExp(
        `\\s*implementation\\s+['"][^'"]*${dep.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[^'"]*['"][^\\n]*\\n?`,
        "g",
      );
    ...
    if (removedAny) {
      console.log(
        "🧹 Removed private SDK dependencies (private SDK not available)",
      );
      hasChanges = true;
    }
  } else {
    // For local builds, handle dependencies normally
    const existingDependencies = requiredDependencies.filter((dep) =>
      buildGradleContent.includes(dep.check),
    );
    const missingDependencies = requiredDependencies.filter(
      (dep) => !buildGradleContent.includes(dep.check),
    );

    if (missingDependencies.length > 0) {
      console.log(
        `➕ Adding missing dependencies: ${missingDependencies.map((d) => d.name).join(", ")}`,
      );

      // Remove any existing dependencies that might be in wrong locations
      for (const dep of requiredDependencies) {
        const wrongLocationRegex = new RegExp(
          `\\s*${dep.dependency.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*`,
          "g",
        );
      ...
      const dependenciesToAdd = missingDependencies
      ...
      const wrongLocationPattern =
        /(if\s*\([^)]+\)\s*\{[\s\S]*?)(implementation\s+['"][^'"]*['"][\s\S]*?)(\}[\s\S]*?)(\})/;
      ...
      if (wrongLocationPattern.test(buildGradleContent)) {
        ...
        const dependenciesToAdd = requiredDependencies
          .map((dep) => `    ${dep.dependency}`)
          .join("\n");
...
async function verifyAppMavenRepository(buildGradlePath, privateSDKPath) {
  ...
  if (!privateSDKAvailable) {
    console.log(
      "⚠️  Private SDK not available - removing private Maven repository references",
    );

    // Remove any existing private Maven repository references
    const privateMavenRegex =
      /maven\s*\{\s*url\s*rootProject\.file\(['"][^'"]*private-sdk[^'"]*['"]\)\s*\}/g;
    if (buildGradleContent.match(privateMavenRegex)) {
      buildGradleContent = buildGradleContent.replace(privateMavenRegex, "");
      fs.writeFileSync(buildGradlePath, buildGradleContent);
    }
  ...
  if (
    buildGradleContent.includes("maven { url rootProject.file(") &&
    buildGradleContent.includes("private-sdk")
  ) {
    return;
  }
